...
object List { // `List` companion object. Contains functions for creating and working with lists.
  ...
  def tail[A](l: List[A]): List[A] = l match {
    case Nil           => sys.error("Trying to return a tail of an empty list")
    case Cons(_, tail) => tail
  }

  def setHead[A](l: List[A], h: A): List[A] = l match {
    case Nil => sys.error("Trying to replace the head of an empty list")
    case Cons(_, tail) =>
      Cons(h, tail) // Discarding the head and returning the new list
  }
